    ...
    fn try_parse(path: &str) -> Result<Option<(Prefix, usize)>, error::ObjectPathError> {
        ...
        if path.starts_with("\\\\?\\") {
            if path.starts_with("\\\\?\\UNC\\") {
                let (server, next) = ObjectPath::find_separator(path, 8, false);
                if next == path.len() {
                ...
                let (share, last) = ObjectPath::find_separator(path, next + 1, false);
                return Ok(Some((
        ...
        if ObjectPath::find_separator(path, 0, true) == ("", 0)
            && ObjectPath::find_separator(path, 1, true) == ("", 1)
        {
            // Starts with two separators.
            let (server, next) = ObjectPath::find_separator(path, 2, true);
            if next < path.len() {
                let (share, last) = ObjectPath::find_separator(path, next + 1, true);
                return Ok(Some((
...
impl fmt::Display for Prefix {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Prefix::UNC(ref server, ref share) => f.pad(&format!("\\\\{}\\{}", server, share)),
...
impl ObjectPath {
    ...
    /// Creates a `ObjectPath` from a `Path` from std.
    pub fn from_std_path(path: &Path) -> Result<ObjectPath, error::ObjectPathError> {
        ...
        if let Some(string) = path.to_str() {
            let mut fspath = ObjectPath::new(string)?;
            if is_dir {
    ...
    /// Converts a `ObjectPath` into a std `PathBuf`.
    ...
    pub fn new<S: AsRef<str>>(from: S) -> Result<ObjectPath, error::ObjectPathError> {
        ...
        let mut pos: usize = 0;
        let mut result: ObjectPath = Default::default();
        ...
        while pos < path.len() {
            let (part, next) = ObjectPath::find_separator(path, pos, any_separator);
    ...
    /// Tests whether this `ObjectPath` is an absolute path.
    pub fn is_absolute(&self) -> bool {
        self.is_absolute
    }

    /// Tests whether this `ObjectPath` is expected to be a directory.
    pub fn is_directory(&self) -> bool {
        self.filename.is_none()
    }

    /// Tests whether this `ObjectPath` is a windows style path.
    ...
    /// target path.
    ///
    /// Both this `ObjectPath` and the target `ObjectPath` must be absolute.
    pub fn relative(&self, target: &ObjectPath) -> Result<ObjectPath, error::ObjectPathError> {
        if !self.is_absolute {
            return Err(error::parse_error(
                &format!("{}", self),
        ...
        let mut relative: ObjectPath = Default::default();
    ...
    pub fn join(&self, path: &ObjectPath) -> Result<ObjectPath, error::ObjectPathError> {
        self.assert_is_normalized();
    ...
    /// Gets the filename for this `ObjectPath` if there is one.
    pub fn filename(&self) -> Option<String> {
        self.filename.clone()
    }

    /// Moves this `ObjectPath` to the named subdirectory.
    ///
    ...
    /// Overwrites the filename for this `ObjectPath`.
...
mod tests {
    ...
    fn test_path_parse_windows() -> Result<(), error::ObjectPathError> {
        let path = ObjectPath::new("C:\\foo\\bar")?;
        assert_eq!(
            path,
            ObjectPath {
        ...
        let path = ObjectPath::new("\\\\bar\\foo/test")?;
        assert_eq!(
            path,
            ObjectPath {
        ...
        let path = ObjectPath::new("\\\\?\\UNC\\bar\\foo\\test")?;
        assert_eq!(
            path,
            ObjectPath {
    ...
    fn test_path_normalize() -> Result<(), error::ObjectPathError> {
        ...
        let path = ObjectPath::new("/foo/baz//diz/.././bar/")?;
        ...
        let path = ObjectPath::new("../baz/../../diz")?;
    ...
    fn test_path_join() -> Result<(), error::ObjectPathError> {
        let base = ObjectPath::new("/foo/bar")?;
        let sub = ObjectPath::new("test/baz")?;
        ...
        let base = ObjectPath::new("/")?;
        let sub = ObjectPath::new("foo/bar/baz")?;
        let joined = base.join(&sub)?;
        assert_eq!(joined.to_string(), "/foo/bar/baz");
        assert!(joined.is_absolute());
        assert!(!joined.is_directory());
    ...
    fn test_path_relative() -> Result<(), error::ObjectPathError> {
        let base = ObjectPath::new("/foo/bar")?;
        let next = ObjectPath::new("/foo/baz")?;
        ...
        let base = ObjectPath::new("/foo/bar")?;
        let next = ObjectPath::new("/foo/bar")?;
        let relative = base.relative(&next)?;
        ...
        let base = ObjectPath::new("/foo/bar/baz")?;
        let next = ObjectPath::new("/foo/bar/bad/gah")?;
        let relative = base.relative(&next)?;
        ...
        let base = ObjectPath::new("/foo/bar/baz")?;
        let next = ObjectPath::new("/foo/bad/gah")?;
        let relative = base.relative(&next)?;
        assert_eq!(relative.to_string(), "../bad/gah");
        assert!(!relative.is_absolute());
        assert!(!relative.is_directory());
        ...
        let base = ObjectPath::new("/foo/bar/baz")?;
        let next = ObjectPath::new("/foo/bar/baz")?;
        let relative = base.relative(&next)?;
        assert_eq!(relative.to_string(), "baz");
        assert!(!relative.is_absolute());
        assert!(!relative.is_directory());
        ...
        let base = ObjectPath::new("/foo/bar/baz")?;
        let next = ObjectPath::new("/foo/bar/baz/gad")?;
        let relative = base.relative(&next)?;
        ...
        let base = ObjectPath::new("/foo/bar/baz/gah/ooh")?;
        let next = ObjectPath::new("/foo/bar/bad/gah")?;
        let relative = base.relative(&next)?;
        assert_eq!(relative.to_string(), "../../bad/gah");
        assert!(!relative.is_absolute());
    ...
    fn test_make_dir() -> Result<(), error::ObjectPathError> {
        fn test_into_dir(path: &str, expected: &str) -> Result<(), error::ObjectPathError> {
            let mut file = ObjectPath::new(path)?;
            file.make_dir();
